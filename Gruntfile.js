...
module.exports = function (grunt) {
  ...
  grunt.registerTask('build-firefox-debug', ['shell:bundle_firefox_debug']);
  grunt.registerTask('build-firefox-prod', ['shell:bundle_firefox_prod']);

  grunt.registerTask('build-chromium-mv2', ['build-chromium-mv2-debug', 'build-chromium-mv2-prod']);
  grunt.registerTask('build-chromium-mv2-debug', ['shell:bundle_chromium_mv2_debug']);
  grunt.registerTask('build-chromium-mv2-prod', ['shell:bundle_chromium_mv2_prod']);

  grunt.registerTask('build-chromium-mv3', ['build-chromium-mv3-debug', 'build-chromium-mv3-prod']);
  grunt.registerTask('build-chromium-mv3-debug', ['shell:bundle_chromium_mv3_debug']);
  grunt.registerTask('build-chromium-mv3-prod', ['shell:bundle_chromium_mv3_prod']);

  grunt.registerTask('build-safari', ['build-safari-debug', 'build-safari-prod']);
  grunt.registerTask('build-safari-debug', ['shell:bundle_safari_debug']);
  grunt.registerTask('build-safari-prod', ['shell:bundle_safari_prod']);
