...
describe("FavoriteResourceService", () => {
  ...
  describe("::addResourceToFavorite", () => {
    it("should add a resource to favorites and update locally", async () => {
      ...
      jest.spyOn(favoriteResourceService.favoriteApiService, "create").mockResolvedValue(favoriteDto);

      // Mock the private updateFavoriteLocally method
      jest.spyOn(favoriteResourceService, "updateFavoriteLocally").mockResolvedValue();
  ...
  describe("::removeResourceFromFavorite", () => {
    it("should remove a resource from favorites and update locally", async () => {
      ...
      jest
        .spyOn(favoriteResourceService.resourceModel, "getById")
        .mockResolvedValue(new ResourceEntity(resourceWithFavorite));

      // Mock the API service delete method
      jest.spyOn(favoriteResourceService.favoriteApiService, "delete").mockResolvedValue();
    ...
    it("should not call delete if resource has no favorite", async () => {
      ...
      jest.spyOn(favoriteResourceService.resourceModel, "getById").mockResolvedValue(new ResourceEntity(resourceDto));
  ...
  describe("::updateFavoriteLocally", () => {
    ...
    it("should throw error if resourceId is not a valid UUID", async () => {
      ...
      await expect(favoriteResourceService.updateFavoriteLocally("invalid-id", null)).rejects.toThrow();
    });
    ...
    it("should throw error if favoriteEntity is not a FavoriteEntity instance", async () => {
      ...
      await expect(favoriteResourceService.updateFavoriteLocally(resourceDto.id, invalidFavorite)).rejects.toThrow();
    });
  });
});
