...
import ExportResourcesFileEntity, {
  ...
  FORMAT_KDBX,
  FORMAT_KDBX_OTHERS,
} from "../../../model/entity/export/exportResourcesFileEntity";
...
describe("ExportResourcesService", () => {
  ...
  beforeEach(async () => {
    ...
    jest
      .spyOn(
        encryptMetadataService.getOrFindMetadataSettingsService.findAndUpdateMetadataSettingsLocalStorageService
        ...
        describe.each([
          ...
          it(`${iteration.version}`, async () => {
            ...
            const resourceType = resourceTypeCollection.find(
              (resourceType) => resourceType.slug === iteration.resourceType,
            );
            const file = {
              format: test.format,
      ...
      describe.each([{ format: FORMAT_KDBX }, { format: FORMAT_KDBX_OTHERS }])(
        ...
        (test) => {
          ...
          async function getKDBXContent(exportResourcesFileEntity, resourceCollection) {
            ...
            exportResourcesFileEntity.exportResources.items[0].totp = new ExternalTotpEntity(
              defaultTotpDto({ secret_key: "THISISASECRET" }),
            );
